...
<title>K&R Car Wash — Staff Scanner</title>
...
maxlength="8"
onkeydown="if(event.key==='Enter') searchPlate()"
/>
...
// Replace this with real API call to your database when ready
const DEMO_MEMBERS = {
'MEMBER-001': { name: 'Carlos Ramirez', vehicle: 'Tesla Model Y', plate: '8ABC123', status: 'active' },
'MEMBER-002': { name: 'Sarah Johnson', vehicle: 'Honda Civic', plate: '7XYZ456', status: 'active' },
'MEMBER-003': { name: 'Mike Torres', vehicle: 'Ford F-150 Raptor', plate: '6DEF789', status: 'inactive' },
'MEMBER-004': { name: 'Jessica Lee', vehicle: 'MINI Cooper', plate: '5GHI012', status: 'active' },
'MEMBER-005': { name: 'David Kim', vehicle: 'Audi A4', plate: '4JKL345', status: 'active' },
};

// Also index by plate for plate search
const BY_PLATE = {};
Object.entries(DEMO_MEMBERS).forEach(([id, m]) => {
BY_PLATE[m.plate.replace(/\s/g,'').toUpperCase()] = { ...m, id };
});
...
// ── SHOW RESULT ───────────────────────────────────────────
function showResult(member, query) {
...
const vehicles = [member.vehicle1, member.vehicle2, member.vehicle3].filter(v => v && v.trim());
document.getElementById('resultVehicle').textContent = notFound ? '' : vehicles.join(' · ');
...
<div class="recent-item ${s.status === 'active' ? 'green' : 'red'}">
<div>
<div class="recent-name">${s.name}</div>
<div class="recent-plate">${s.plate}</div>
</div>
<div style="text-align:right;">
<div class="recent-status ${s.status === 'active' ? 'green' : 'red'}">${s.status.toUpperCase()}</div>
